<template>
    <div class="column is-multiline">
         <div class="column is-12">
             <h2 class="is-size-2 has-text-centered">Распределение {{ report_metric }} по преподавателям </h2>
         </div>
         <div class="container box">
             <p class="has-text-centered">Отчет по преподавателям за {{year}} год</p>
             <div class="columns">
             ...
             <strong>Всего преподавателей: {{ sum_tutors.length }}</strong> <br>
             <strong>Всего часов: {{  getSumHours }}</strong> <br>
             <strong>Всего выплачено средств: {{  getSumIncomes }}</strong>
             <table>
                 <tr>
                     <th class="has-text-centered ">ФИО</th>
                     <th class="has-text-centered">Количество часов</th>
                     <th class="has-text-centered">Выплачено средств, руб.</th>

                 </tr>
                 <tr v-for="tutor in sum_tutors" :key="tutor.id">
                     <th class="has-text-centered has-text-weight-light"> {{ tutor.tutor__name }}</th>
...
export default {
  ...
  computed: {
    ...
    getTutorsNames: function () {
      return this.sum_tutors.map(a => a.tutor__name)
    },
      ...
      axios
        ...
        .then(response => {
          ...
          this.sum_tutors = response.data.data.reduce((acc, cur) => {
            let inacc = false
            acc.forEach(o => {
              if (o.tutor__name === cur.tutor__name) {
